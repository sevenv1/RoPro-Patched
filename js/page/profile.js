...
async function postReputation(type) {
	...
	return new Promise(resolve => {
		...
		chrome.runtime.sendMessage({greeting: "PostURL", form: true, url:"https://api.ropro.io/postReputation.php", jsonData: json}, 
			function(data) {
				resolve(data)
			}
		)
...
async function addMutuals() {
	friendsBar = document.getElementsByClassName('profile-header-details')[0].getElementsByClassName('profile-header-social-counts')[0]
	...
	li.innerHTML = `<a class="profile-header-social-count" href="https://www.roblox.com/users/${parseInt(getIdFromURL(location.href))}/friends#!/friends#mutuals" title="Mutual${numMutuals == 1 ? "" : "s"}"><span class="MuiTypography-root web-blox-css-tss-hzyup-Typography-body1-Typography-root MuiTypography-inherit web-blox-css-mui-clml2g"><b>${parseInt(numMutuals)}</b> Mutual${numMutuals == 1 ? "" : "s"}</span></a>`
	setTimeout(function(){
...
async function addRoProInfo(tier, user_since, subscribed_for) {
	...
	header = document.getElementsByClassName('profile-header-title-container')[0]
...
function addDiscord(discord) {
	caption = document.getElementsByClassName('profile-header-names')[0]
	margin = 145
	size = "15px"
...
async function mainProfile() {
	if (location.href.includes("/profile")) {
		...
		if (userID != undefined && userID != 0) {
			...
			caption.insertBefore(reputationDiv, caption.getElementsByClassName('profile-header-details')[0])
			myId = await getStorage("rpUserID")
