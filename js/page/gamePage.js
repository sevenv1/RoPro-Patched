...
async function fetchServerInfo(placeID, servers) {
	return new Promise(resolve => {
		chrome.runtime.sendMessage({greeting: "PostURL", wrap_json: true, url:"https://api.ropro.io/getServerInfo.php?form", jsonData: {'placeID':placeID, 'servers': servers}}, 
			function(data) {
				resolve(data)
			}
		)
...
async function loadServerPage(gameID, index, reverse){
	...
	for (i = index; i < Math.min(serverPage.length, index + 8); i++) {
		...
		var shortServerId = "";
		if (server.id.split("-").length >= 3) {
			shortServerId = server.id.split("-")[1] + "-" + server.id.split("-")[2]
		} else {
			shortServerId = "Null"
		}
		if (isCardList) {
			serversHTML += `<li class="rbx-public-game-server-item col-md-3 col-sm-4 col-xs-6" data-gameid="${gameId}" data-placeid="${placeId}"><div class="card-item">
			...
			<span data-placeid="${placeId}"><button type="button" class="ropro-custom-server-list-item btn-full-width btn-control-xs rbx-public-game-server-join game-server-join-btn btn-primary-md btn-min-width" onclick='if(!document.body.classList.contains("ropro-cloud-play-activated")) {Roblox.GameLauncher.joinGameInstance(${placeId}, "${gameId}")}'>Join</button></span>
			<div class="server-id-text text-info xsmall" style="float: left;">ID: ${stripTags(shortServerId)}</div>
			</div></div></li>`
		} else {
			serversHTML += `<li class="stack-row rbx-public-game-server-item ropro-checked" data-gameid="${gameId}" data-placeid="${placeId}"><div class="section-header">
							...
							<div class="section-left rbx-public-game-server-details">
								...
								<a class="btn-full-width btn-control-xs rbx-public-game-server-join" href="#" data-gameid="${gameId}" data-placeid="${placeId}" onclick='if(!document.body.classList.contains("ropro-cloud-play-activated")) {Roblox.GameLauncher.joinGameInstance(${placeId}, "${gameId}")}'>Join</a></div>
...
function addServerFilters(placeId, maxPlayerCount) {
	...
	serverContainer = document.getElementById('rbx-public-running-games')
	...
	document.getElementById('reverseOrderButton').addEventListener('click', async function() {
		if (document.getElementById('loadingBar') == null) {
			...
			document.getElementById('rbx-public-running-games').getElementsByClassName('container-header')[0].appendChild(div.childNodes[0])
		}
		document.getElementById('rbx-public-game-server-item-container').innerHTML = ``
		if (document.getElementsByClassName('rbx-public-running-games-footer').length > 0) {
			document.getElementsByClassName('rbx-public-running-games-footer')[0].style.display = "none"
		}
		if (document.getElementsByClassName('ropro-running-games-footer').length > 0) {
	...
	document.getElementById('reverseOrderButtonMini').addEventListener('click', async function() {
		...
		document.getElementById('rbx-public-running-games').getElementsByClassName('container-header')[0].scrollIntoView()
	})
...
async function liveCounters() {
	async function loadCounters() {
		...
		if (upvotes != null) {
			...
			upvotes.innerHTML = addCommas(parseInt(votes.upVotes))
		}
		if (downvotes != null) {
			downvotes.style.fontSize = "11px"
			downvotes.innerHTML = addCommas(parseInt(votes.downVotes))
		}
		if (favorites != null) {
			favorites.innerHTML = addCommas(parseInt(favoritesData.favoritesCount))
		}
	}
...
async function addEmbeds(sectionContent, myUniverseId) {
	...
	for (i = 0; i < socialLinks.length; i++) {
		if (socialLinks[i].type == "Twitter" && await fetchSetting("gameTwitter") && typeof socialLinks[i] != 'undefined') {
			twitterUrl = socialLinks[i].url
			twitterProfile = stripTags(twitterUrl.split('://x.com/')[1])
			div = document.createElement('div')
...
async function createInviteLink(elem, serverid, placeid) {
	...
	if (elem.classList.contains('create-server-invite-button')) {
		pos = "top: -120px; left: -97px;"
	} else {
		pos = "top: -120px; left: -107px;"
	}
	inviteHTML = `<div uib-popover-template-popup="" uib-title="" class="dark-theme tradeinfocard popover ng-scope ng-isolate-scope bottom people-info-card-container card-with-game fade in server-invite-link-box" tooltip-animation-class="fade" uib-tooltip-classes="" ng-class="{ in: isOpen }" style="filter: drop-shadow(rgb(0, 0, 0) 0px 0px 1px); ${pos} width: 220px; height: 105px;border-radius:10px;">
	<h2 style="padding-bottom:5px;padding-top:0px;border-bottom: 2px solid #FFFFFF;font-family:HCo Gotham SSm;color:white;font-size:30px;top:25px;left:25px;width:200px;margin:auto;text-align:center;">
			<p style="color:white;display:inline-block;font-size:15px;font-weight:650;">RoPro Share Link</p>
		</h2><div style="width:200px;margin:auto;margin-top:10px;border-radius:10px;" class="input-group server-invite-link-input"><span style="float: right; width: 100%; height: 22.5px; visibility: initial !important;" class="spinner spinner-default"></span></div><div style="left: 135px; transform: rotate(180deg) scale(2); top: initial; bottom: -10px;" class="ropro-arrow arrow"></div>
	...
	var invite = await createInvite(myUniverseId, serverid);
	invite = invite.replace("https://", "").replace("ropro.io/join/", "ro.pro/")
	elem.getElementsByClassName('server-invite-link-input')[0].innerHTML = `<form><div class="form-has-feedback"><p class="copied-to-clipboard" style="position:absolute;top:-30px;left:55px;background-color:#111212;border-radius:10px;padding:5px;font-size:12px;display:none;">Copied to clipboard.</p><img src="${chrome.runtime.getURL('/images/ropro_icon_white.png')}" style="width:20px;height:20px;margin-left:10px;position:absolute;top:9px;"/><input style="font-size:20px;border-radius:10px;margin-bottom:5px;opacity:1;cursor:pointer;font-weight:100;color:white;padding-left:37px;padding-right:0;" id="navbar-search-input" class="form-control input-field new-input-field copy-clipboard-button" placeholder="Search" maxlength="120" autocomplete="off" autocorrect="off" autocapitalize="off" spellcheck="false" value="${stripTags(invite)}" readonly></div></form><div class="input-group-btn" style="margin:-40px;"><button style="right:10px;left:initial;pointer-events:none;" id="copy-btn" class="input-addon-btn" type="submit"><img src="${chrome.runtime.getURL('/images/copy.png')}" style="width:18px;height:18px;filter:invert(0.8);display:none;"></button></div>`
...
function addServerInviteButton(elem, serverid, placeid) {
	...
	if (elem.classList.contains('rbx-friends-game-server-item')) {
		serverInviteButtonHTML = `<a style="width:30%;margin-left:2%;position:relative!important;" class="btn-full-width btn-control-xs create-server-link" data-placeid="${parseInt(placeid)}" data-serverid="${stripTags(serverid)}">Share</a>`
		div = document.createElement('div')
		...
		serverInviteButtonHTML = `<a style="width:30%;margin-left:2%;position:relative!important;bottom:-6px;" class="btn-full-width btn-control-xs create-server-link" data-placeid="${parseInt(placeid)}" data-serverid="${stripTags(serverid)}">Share</a>`
		div = document.createElement('div')
		div.innerHTML = serverInviteButtonHTML
		button = div.childNodes[0]
		elem.getElementsByClassName('rbx-public-game-server-join')[0].style.width = elem.getElementsByClassName('rbx-public-game-server-join')[0].classList.contains('ropro-custom-server-list-item') ? "65%" : "68%"
		if (elem.getElementsByClassName('rbx-public-game-server-details').length > 0) {
			var gameServerDetails = elem.getElementsByClassName('rbx-public-game-server-details')[0];
			if (gameServerDetails.getElementsByClassName('server-id-text').length > 0) {
				gameServerDetails.insertBefore(button, gameServerDetails.getElementsByClassName('server-id-text')[0])
			} else {
				gameServerDetails.appendChild(button)
			}
		} else if (elem.getElementsByClassName("rbx-public-game-server-details'").length > 0) {
			var gameServerDetails = elem.getElementsByClassName("rbx-public-game-server-details'")[0];
			if (gameServerDetails.getElementsByClassName('server-id-text').length > 0) {
				gameServerDetails.insertBefore(button, gameServerDetails.getElementsByClassName('server-id-text')[0])
			} else {
...
function addMostRecentServer(placeID, serverID, userID, time, serverActive) {
	...
	if (serverActive) {
		...
		<div style="float:left;" class="text-info rbx-game-status rbx-public-game-server-status"><b>Last Played:</b><img style="background-image:none;margin:0px;margin-top:-2px;margin-bottom:1px;transform:scale(1);border:none;margin-left:5px;margin-right:5px;width:15px;height:15px;" src="${chrome.runtime.getURL(`/images/timer${theme == "dark" ? "_dark" : "_light"}.svg`)}" class="info-label icon-pastname">${stripTags(timeString)}</div><div style="float:left;" class="text-info rbx-game-status rbx-public-game-server-status"></div>
		<div class="rbx-public-game-server-alert hidden"><span class="icon-remove"></span>Slow Game</div><a style="width:89%;float:left;" class="ropro-game-server-join btn-full-width btn-control-xs" data-serverid="${stripTags(serverID)}" data-placeid="${parseInt(placeID)}" onclick="if(!document.body.classList.contains('ropro-cloud-play-activated')) {Roblox.GameLauncher.joinGameInstance(${parseInt(placeID)}, &quot;${stripTags(serverID)}&quot;)}">Rejoin Server</a><a style="width:9.5%;float:right;margin-right:1%;position:relative!important;" class="btn-full-width btn-control-xs create-server-invite-button" data-serverid="${stripTags(serverID)}" data-placeid="${parseInt(placeID)}">Share</a></div><div style="width:5%;margin-top:13px;margin-right:2.1%;float:right;" class="section-right rbx-public-game-server-players"><span style="transform:scale(1.3);" class="avatar avatar-headshot-sm player-avatar"><a class="avatar-card-link"><img class="avatar-card-image ropro-player-thumbnail-${parseInt(userID)}"></a></span></div></li>`	
		...
		<div style="float:left;" class="text-info rbx-game-status rbx-public-game-server-status"><b>Last Played:</b><img style="background-image:none;margin:0px;margin-top:-2px;margin-bottom:1px;transform:scale(1);border:none;margin-left:5px;margin-right:5px;width:15px;height:15px;" src="${chrome.runtime.getURL(`/images/timer${theme == "dark" ? "_dark" : "_light"}.svg`)}" class="info-label icon-pastname">${stripTags(timeString)}</div><div style="float:left;" class="text-info rbx-game-status rbx-public-game-server-status"></div>
		<div class="rbx-public-game-server-alert hidden"><span class="icon-remove"></span>Slow Game</div><a class="ropro-game-server-join btn-full-width btn-control-xs" data-placeid="${parseInt(placeID)}" onclick="if(!document.body.classList.contains('ropro-cloud-play-activated')) {Roblox.GameLauncher.joinGameInstance(${parseInt(placeID)}, &quot;${stripTags(serverID)}&quot;)}">Server No Longer Active</a></div><div style="width:9%;margin-top:13px;margin-left:1%;" class="section-right rbx-public-game-server-players"><span style="transform:scale(1.3);" class="avatar avatar-headshot-sm player-avatar"><a class="avatar-card-link"><img class="avatar-card-image ropro-player-thumbnail-${parseInt(userID)}"></a></span></div></li>`	
	}
...
async function getServerInfo(queue) {
	...
	for (var i = 0; i < serverInfo.length; i++) {
		...
		if (!elem.classList.contains('stack-row') && elem.getElementsByClassName('ropro-uptime-info').length == 0) {
			var uptimeDiv = document.createElement('div');
			uptimeDiv.innerHTML = `<div style="margin-top:6px;float:right;" class="ropro-uptime-info text-info xsmall"><span style="font-weight:initial;">Uptime: ${stripTags(timeSince)}</span></div>`
			if (joinButton.parentNode.parentNode.getElementsByClassName('server-id-text').length > 0) {
				joinButton.parentNode.parentNode.getElementsByClassName('server-id-text')[0].style.float = "left";
			}
			joinButton.parentNode.parentNode.appendChild(uptimeDiv.childNodes[0]);
		}
	}
